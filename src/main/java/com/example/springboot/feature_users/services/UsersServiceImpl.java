...
public class UsersServiceImpl implements UsersService {
     ...
     */
    @Autowired
    public UsersServiceImpl(UsersDao usersDao, AuthenticationManager authenticationManager, JwtUtils jwtUtil) {
     ...
     * @return A ResponseEntity containing an ApiResponse with the JWT token upon success,
    ...
    public ResponseEntity<ApiResponse> login(Users user) {
        try {
            ...
            return ResponseEntity.ok(new ApiResponse(true, null, SUCCESSFUL_LOGIN, null, tokenResponse));

        } catch (BadCredentialsException e) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(new ApiResponse(false, INVALID_CREDENTIALS, INVALID_USERNAME_PASSWORD, null, null));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse(false, INTERNAL_SERVER_ERROR, UNEXPECTED_ERROR, e.getMessage(), null));
        }
    }
    ...
    public ResponseEntity<ApiResponse> register(Users user) {
        ...
        return ResponseEntity.ok(new ApiResponse(true, null, SUCCESSFUL_REGISTRATION, null, save(user)));
    }
}
